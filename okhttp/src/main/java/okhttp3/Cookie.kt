...
class Cookie private constructor(
  ...
  class Builder {
    ...
    fun expiresAt(expiresAt: Long) = apply {
      ...
      if (expiresAt <= 0) expiresAt = java.lang.Long.MIN_VALUE
  ...
  internal fun toString(forObsoleteRfc2965: Boolean): String {
    return buildString {
      ...
      if (persistent) {
        if (expiresAt == java.lang.Long.MIN_VALUE) {
          append("; max-age=0")
        } else {
  ...
  companion object {
    ...
    internal fun parse(currentTimeMillis: Long, url: HttpUrl, setCookie: String): Cookie? {
      ...
      if (deltaSeconds == java.lang.Long.MIN_VALUE) {
        expiresAt = java.lang.Long.MIN_VALUE
      } else if (deltaSeconds != -1L) {
        val deltaMilliseconds = if (deltaSeconds <= java.lang.Long.MAX_VALUE / 1000) {
          deltaSeconds * 1000
        } else {
    ...
    private fun parseMaxAge(s: String): Long {
      try {
        val parsed = java.lang.Long.parseLong(s)
        return if (parsed <= 0L) java.lang.Long.MIN_VALUE else parsed
      } catch (e: NumberFormatException) {
        // Check if the value is an integer (positive or negative) that's too big for a long.
        if (s.matches("-?\\d+".toRegex())) {
          return if (s.startsWith("-")) java.lang.Long.MIN_VALUE else java.lang.Long.MAX_VALUE
        }
        throw e
      }
    }
    ...
    private fun parseDomain(s: String): String {
      ...
      return canonicalizeHost(s) ?: throw IllegalArgumentException()
    }
    ...
    fun parseAll(url: HttpUrl, headers: Headers): List<Cookie> {
      ...
      var i = 0
      val size = cookieStrings.size
      while (i < size) {
        val cookie = Cookie.parse(url, cookieStrings[i])
        if (cookie == null) {
          i++
          continue
        }
